/*
  Spotify live player for Arduino UNO R4 WiFi + 240x240 ST7789
  ...
  TFT wiring (hardware SPI)
    TFT VCC -> use the voltage required by your module
    TFT GND -> GND
    TFT SCL -> D13 / SCK
    TFT SDA -> D11 / MOSI
    TFT CS  -> D9
    TFT DC  -> D8
    TFT RST -> D7
    TFT BL  -> module supply (or a suitable backlight circuit)
...
bool readHttpResponseHeaders(WiFiSSLClient& client, HttpResponse& response);

bool readNetworkLine(WiFiSSLClient& client, String& line) {
  line = "";
  line.reserve(160);
...
class HttpBodyStream : public Stream {
 public:
  HttpBodyStream(WiFiSSLClient& client, long contentLength, bool chunked)
...
bool renderAlbumJpeg(uint8_t* jpegData, size_t jpegLength) {
  ...
  while (JpegDec.read()) {
    ...
    int16_t drawWidth = min(mcuWidth, imageWidth - JpegDec.MCUx * mcuWidth);
    int16_t drawHeight = min(mcuHeight, imageHeight - JpegDec.MCUy * mcuHeight);
...
void setup() {
  ...
  drawStatusScreen("SPOTIFY TFT", "Starting Arduino UNO R4 WiFi...");
  Serial.println(F("MakerLab Spotify TFT starting"));
